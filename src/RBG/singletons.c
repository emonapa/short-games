...
// Tisk
// -----------------------------------------------------------------
static void game_print_raw_recursive(Game *G) {
    if (!G) return;
    ...
    else if (G == val_star) { printf("*"); return; }
    ...
    for (int i = 0; i < G->L_count; i++) {
        game_print_raw_recursive(G->left[i]);
        if (i < G->L_count - 1) printf(", ");
    }

    printf(" | ");
    for (int i = 0; i < G->R_count; i++) {
        game_print_raw_recursive(G->right[i]);
...
static void game_to_string_recursive(Game *G) {
    if (!G) return;
    ...
    else if (G == val_star) { buffer_append("*"); return; }
    ...
    buffer_append("{");
    for (int i = 0; i < G->L_count; i++) {
        game_to_string_recursive(G->left[i]);
        if (i < G->L_count - 1) buffer_append(", ");
    }
    buffer_append(" | ");
    for (int i = 0; i < G->R_count; i++) {
        game_to_string_recursive(G->right[i]);
