...
from PySide6.QtWidgets import QFileDialog, QGraphicsTextItem, QGraphicsDropShadowEffect
...
class InteractiveEdgeItem(QGraphicsPathItem):
    def __init__(self, scene, edge_idx, color, *args, **kwargs):
        ...
        # Zapneme přijímání hover eventů
...
class GraphScene(QGraphicsScene):
    def __init__(self, solver: Optional[hb_solver.HBSolver], theme=None) -> None:
        ...
        # Mapování, které vrcholy leží na zemi
        ...
        self.hint_timer.setSingleShot(True) # Vykoná se jen jednou
        self.hint_timer.timeout.connect(self._process_delayed_hint)
        self.pending_hint_data = None
        # -------------------------------------------------------------
        self.edit_mode = False # muzu nicit jake chci hrany
        self.on_build_color_changed = None # zmenila se barva scrollovanim
        self.trash_bin = [] # pro hrany ktere mazu

        self._draw_ground()

    def _draw_ground(self) -> None:
        # Tmavý obdélník země od self.ground_y až úplně dolů
    ...
    def mousePressEvent(self, event) -> None:
        ...
        # kliknutí pod zem přicucne Y přesně na úroveň země
        ...
        # start ninja fruit animaci (musí začít ve vzduchu, ne na zemi)
    ...
    def _execute_cut(self, cut_idx: int) -> None:
        if not self.solver:
            print("C-Solver neni načten, nelze provest korektní cut/cleanup")
            return
        ...
        # ----------------- Mazání hran -----------------
        # musi to byt takto special, protoze Qt nema problem hodit segfault,
        # kdyz smazeme hranu chvilku predtim nez jej plne zpracuje
        ...
        for idx in to_remove:
            ...
            # Objekty pouze zneviditelníme a uspíme.
            # mazat ze scény je budeme chvili
            item.hide()
        ...
        # Qt vykoná tuto funkci,
        # až když si je na 100 % jisté, že myš už s objekty nic nedělá.
    ...
    # funkce ciste kvuli Qt mazani hran a jeji hazeni segfaultu
    def _safe_remove_items(self, items_to_trash):
        """odstraní objekty ze scény až poté, co Qt dokončí event loop."""
        for item in items_to_trash:
            if item.scene() == self:
                self.removeItem(item)

        self.trash_bin.extend(items_to_trash)
        # Udržíme koš malý, ať nám nežere zbytečně paměť
    ...
    def _add_edge(self, u: int, v: int, color: int) -> bool:
        ...
        # Pokud je vrchol na zemi, do C pošleme natvrdo index 0
        ...
        # Trida InteractiveEdgeItem kvuli bublinam
    ...
    def show_hint_for_edge(self, edge_idx, color, pos):
        ...
        # Jen si uložíme data a spustíme odpočet (400 ms)
        self.pending_hint_data = (edge_idx, color, pos)
        self.hint_timer.start(400)

    def hide_hint(self):
        # Pokud myší uhneme dřív, než timer vyprší, zrušíme výpočet
        self.hint_timer.stop()
        self.floating_hint.hide()

    def _process_delayed_hint(self):
        """Tato funkce se spustí, až když myš stojí na místě 400 ms"""
        ...
        # Výpočet hodnoty v C solveru
        temp_mask = ((1 << int(self.g.num_edges)) - 1) & ~(1 << edge_idx)
        new_mask = self.solver.cleanup_position(self.g, temp_mask)
        try:
            game_ptr = self.solver.solve_with_components(self.g, new_mask)
            val_str = self.solver.get_game_value_string(game_ptr)
    ...
    def get_best_moves(self):
        ...
        for idx in valid_moves:
            ...
            new_mask = self.solver.cleanup_position(self.g, temp_mask)
            # uplne v pohode ze se nam pozice budou extremne opakovat,
            # mame je uz v mezipameti
            game_ptr = self.solver.solve_with_components(self.g, new_mask)
    ...
    def update_auras(self):
        # Vymaže staré aury a vykreslí nové na nejlepší hrany.
        # 1. zhasneme všechny hrany
        for e in self.edge_items:
            e.item.setGraphicsEffect(None)

        # 2. pokud jsou hinty vypnuté, končíme
        if not self.hints_active or self.edit_mode:
            return

        # 3. zjistíme nejlepší tahy a rozsvítíme je
        best_moves = self.get_best_moves()
        for idx in best_moves:
            effect = QGraphicsDropShadowEffect()
            effect.setBlurRadius(20) # Síla aury
            effect.setColor(QColor(255, 255, 255, 255)) # bílá
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        try:
            ...
            self.solver = None
            print(f"Varování: C-Solver nenalezen, UI pobezi bez matematiky: {e}")
        ...
        self.edit_btn.setToolTip("Edit mód: Lze smazat cokoliv")
        ...
        self.edu_btn.setToolTip("Education mód: Analýza CGT")
    ...
    def closeEvent(self, event):
        """Zavolá se automaticky při zavírání okna."""
        if self.solver:
            self.solver.free_all()
            print("C-Solver paměť byla čistě uvolněna.")
        event.accept()
    ...
    def solve_current(self) -> None:
        if not self.solver:
            self.result_lbl.setText("Chyba: Solver není načten.")
            return

        try:
            self.solver.initialize(None)
            live_mask = self.scene.compute_live_mask_all_edges()
            val = self.solver.solve_with_components(self.scene.g, live_mask)

            if not val:
                 self.result_lbl.setText("Value: Null pointer returned")
                 return
            ...
            if g_geq_0 and not zero_geq_g:
                res_str = "G > 0 (Vyhrává Modrý / Left)"
            elif not g_geq_0 and zero_geq_g:
                res_str = "G < 0 (Vyhrává Červený / Right)"
            elif g_geq_0 and zero_geq_g:
                res_str = "G = 0 (Vyhrává 2. hráč / Second)"
            else:
                res_str = "G || 0 (Fuzzy pozice, vyhrává 1. hráč / First)"

            self.result_lbl.setText(f"Outcome: {res_str}")
        except Exception as e:
            self.result_lbl.setText(f"Outcome: error ({e})")
    ...
    def _update_color_button_ui(self, color: int) -> None:
        """Přepíše styl (barvu) tlačítka na základě vybrané barvy."""
    ...
    def toggle_build_color(self) -> None:
        """Tato funkce se zavolá při kliknutí myší na tlačítko."""
        colors = [hb_solver.EDGE_BLUE, hb_solver.EDGE_RED, hb_solver.EDGE_GREEN]
        current_idx = colors.index(self.scene.current_color)

        # Posune barvu vždy o 1 dopředu
    ...
    def step_forward(self):
        self.scene.hide_hint()
        # Pokud máme něco v redu, jdeme dopředu přes to
        if self.history.redo_stack:
            self.history.redo()
            return

        # Pokud ne, zahrajeme nejlepší tah (BOTA!)
