...
class HistoryManager:
    ...
    def load_state(self, state):
        ...
        self.scene.g.num_vertices = hb_converter.c_uint8(idx)
...
class EducationManager:
    ...
    def update_overlay(self):
        ...
        converter = self.scene.converter
        g = self.scene.g
        live_mask = self.scene.compute_live_mask_all_edges()
        is_left = (self.scene.player_to_move == hb_converter.EDGE_BLUE)

        root_game = converter.convert(g, live_mask)
        ...
        for idx in valid_moves:
            ...
            mask_after_i = converter.cleanup_position(g, temp_mask)

            g_i = converter.convert(g, mask_after_i)
        ...
        for idx in valid_moves:
            ...
            for j in range(g.num_edges):
                ...
                c = g.edges[j].color
                if c == opp_color or c == hb_converter.EDGE_GREEN:
                    temp_mask_j = mask_after_i & ~(1 << j)
                    mask_after_j = converter.cleanup_position(g, temp_mask_j)

                    g_ij = converter.convert(g, mask_after_j)
                    #g_ij = cached_convert(mask_after_j)
                    if g_ij is None:
                        print("CACHE MISS?????????????")
        ...
        for idx in valid_moves:
            ...
            for k in valid_moves:
                ...
                is_dom = False
                if is_left:
                    if converter.game_geq(g_k, g_i) and not converter.game_geq(g_i, g_k):
                        is_dom = True
                else:
                    if converter.game_geq(g_i, g_k) and not converter.game_geq(g_k, g_i):
                        is_dom = True
